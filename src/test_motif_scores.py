...
from motif_scores import top_n_kmer_scores
...
class TestTopNKmerScores(TestCase):
    ...
    def test_one_score_one_k(self):
        ...
        result = top_n_kmer_scores(seqs, mut_effects, pwms, n=1)

        self.assertEqual(1, len(result))
        self.assertEqual(1, len(result[0]))
        # Note: relying on arbitrary property of sort.
        # Could just as well be 'GGG' or one of 6 other 3-mers.
        np.testing.assert_array_equal(one_hot("AAA"), result[0][0][0]) 
        self.assertEqual(3, result[0][0][1]) 
    ...
    def test_multiple_scores_one_k(self):
        ...
        result = top_n_kmer_scores(seqs, mut_effects, pwms, n=2)

        self.assertEqual(1, len(result))
        self.assertEqual(2, len(result[0]))
        print(result[0])
        self.assertEqual(3, result[0][0][1]) 
        self.assertEqual(3, result[0][1][1]) 
        np.testing.assert_array_equal(one_hot("AAA"), result[0][0][0]) 
        np.testing.assert_array_equal(one_hot("GAA"), result[0][1][0]) 
