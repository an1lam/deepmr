...
unconfounded_global_ces_plot <- 
  ...
  geom_point(size = 2) +
  geom_errorbar(aes(ymin = ce_est - 2 * sqrt(ce_tau2), ymax = ce_est + 2 * sqrt(ce_tau2))) +
  geom_abline(slope=1) +
...
results_summary <- imap(metrics_tables, function (global_metrics, name) {
  ce_lower <- global_metrics$ce_est - 2 * sqrt(global_metrics$ce_tau2)
  ce_upper <- global_metrics$ce_est + 2 * sqrt(global_metrics$ce_tau2)
  true_ce_captured <- (
