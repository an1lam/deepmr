...
mr_results <- foreach(exp_tf=exp_tfs, out_tf=out_tfs, .combine=cbind) %do% {
  sim_eff_sizes_fname <- str_c(exp_tf, out_tf, "effect_sizes_v2.csv", sep="_")
  seq_eff_sizes <- 
...
seq_eff_sizes <- 
  read.csv(file.path(params$sim_results_dir, sim_eff_sizes_fname)) %>%
  drop_na()
sample_misclassified_veffs <- subset(seq_eff_sizes, seq_num == 18)
...
mr_results <- mr_lasso(MRInputObject)
mr_results
```