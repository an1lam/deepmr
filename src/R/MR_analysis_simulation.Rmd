...
```{r}
data_dir <- "../../dat/sim/"
results_dir <- "../../dat/sim/res/"
exposure_tf <- params$exposure_tf
outcome_tf <- params$outcome_tf

results_fname <- str_c(exposure_tf, outcome_tf, "effect_sizes_v4.csv", sep = "_")
...
n_seqs <- max(seq_predictions["seq_num"], 100)
...
max_idx <- as.integer(length(sorted_idxs)) - 1
